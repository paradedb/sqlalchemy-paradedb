...
def test_match_any_or_semantics(mock_session):
    ...
    stmt = select(MockItem.id).where(
        search.match_any(MockItem.description, "running", "wireless")
    )
...
def test_match_all_and_semantics(mock_session):
    ...
    stmt_all = select(MockItem.id).where(
        search.match_all(MockItem.description, "running", "shoes")
    )
    stmt_any = select(MockItem.id).where(
...
def test_phrase_match(mock_session):
    ...
    stmt = select(MockItem.id).where(
        search.phrase(MockItem.description, "running shoes")
    )
    assert_uses_paradedb_scan(mock_session, stmt, index_name="mock_items_bm25_idx")
    ids = _ids(mock_session, stmt)
    assert len(ids) > 0
...
def test_phrase_with_slop(mock_session):
    ...
    stmt_exact = select(MockItem.id).where(
        search.phrase(MockItem.description, "running shoes")
    )
    stmt_slop = select(MockItem.id).where(
        search.phrase(MockItem.description, "running shoes", slop=2)
    )
...
def test_regex_match(mock_session):
    ...
    stmt = select(MockItem.id).where(
        search.regex(MockItem.description, "run.*")
    )
    assert_uses_paradedb_scan(mock_session, stmt, index_name="mock_items_bm25_idx")
    ids = _ids(mock_session, stmt)
    assert len(ids) > 0
...
def test_fuzzy_distance_1(mock_session):
    ...
    stmt = select(MockItem.id).where(
        search.fuzzy(MockItem.description, "runnning", distance=1)
    )
    assert_uses_paradedb_scan(mock_session, stmt, index_name="mock_items_bm25_idx")
    ids = _ids(mock_session, stmt)
...
def test_fuzzy_distance_2(mock_session):
    ...
    stmt_d1 = select(MockItem.id).where(
        search.fuzzy(MockItem.description, "runnning", distance=1)
    )
    stmt_d2 = select(MockItem.id).where(
        search.fuzzy(MockItem.description, "runnning", distance=2)
    )
...
def test_fuzzy_with_prefix(mock_session):
    ...
    stmt = select(MockItem.id).where(
        search.fuzzy(MockItem.description, "runn", distance=0, prefix=True)
    )
    assert_uses_paradedb_scan(mock_session, stmt, index_name="mock_items_bm25_idx")
    ids = _ids(mock_session, stmt)
    assert len(ids) > 0
...
def test_fuzzy_with_transpose_cost_one(mock_session):
    ...
    stmt = select(MockItem.id).where(
        search.fuzzy(MockItem.description, "rnnuing", distance=2, transpose_cost_one=True)
    )
    assert_uses_paradedb_scan(mock_session, stmt, index_name="mock_items_bm25_idx")
    ids = _ids(mock_session, stmt)
...
def test_fuzzy_with_boost(mock_session):
    ...
    stmt_boost = select(MockItem.id).where(
        search.fuzzy(MockItem.description, "runnning", distance=1, boost=2.0)
    )
...
def test_parse_conjunction_mode_narrows_results(mock_session):
    ...
    stmt_default = select(MockItem.id).where(
        search.parse(MockItem.id, "description:running shoes", lenient=True)
    )
...
def test_phrase_prefix_basic(mock_session):
    ...
    stmt = select(MockItem.id).where(
        search.phrase_prefix(MockItem.description, ["running", "sh"])
    )
    assert_uses_paradedb_scan(mock_session, stmt, index_name="mock_items_bm25_idx")
    ids = _ids(mock_session, stmt)
    assert len(ids) > 0
...
def test_regex_phrase_basic(mock_session):
    ...
    stmt = select(MockItem.id).where(
        search.regex_phrase(MockItem.description, ["run.*", "shoe.*"])
    )
    assert_uses_paradedb_scan(mock_session, stmt, index_name="mock_items_bm25_idx")
    ids = _ids(mock_session, stmt)
    assert len(ids) > 0
...
def test_regex_phrase_with_slop(mock_session):
    ...
    stmt_slop = select(MockItem.id).where(
        search.regex_phrase(MockItem.description, ["run.*", "shoe.*"], slop=2)
    )
    assert_uses_paradedb_scan(mock_session, stmt_slop, index_name="mock_items_bm25_idx")
    ids_base = _ids(mock_session, stmt_base)
    ids_slop = _ids(mock_session, stmt_slop)
...
def test_near_unordered(mock_session):
    ...
    stmt = select(MockItem.id).where(
        search.near(MockItem.description, "running", "shoes", distance=3)
    )
    assert_uses_paradedb_scan(mock_session, stmt, index_name="mock_items_bm25_idx")
    ids = _ids(mock_session, stmt)
    assert len(ids) > 0
...
def test_near_ordered_is_subset_of_unordered(mock_session):
    ...
    stmt_unordered = select(MockItem.id).where(
        search.near(MockItem.description, "running", "shoes", distance=3)
    )
...
def test_proximity_expr_chain_unordered(mock_session):
    ...
    stmt = select(MockItem.id).where(
        search.proximity(MockItem.description, prox)
    )
    assert_uses_paradedb_scan(mock_session, stmt, index_name="mock_items_bm25_idx")
    ids = _ids(mock_session, stmt)
    assert isinstance(ids, set)
...
def test_boost_does_not_change_result_set(mock_session):
    ...
    stmt_base = select(MockItem.id).where(
        search.match_any(MockItem.description, "shoes")
    )
    stmt_boost = select(MockItem.id).where(
...
def test_more_like_this_by_document_id(mock_session):
    ...
    shoe_items = list(mock_session.scalars(
    ...
    stmt = select(MockItem.id).where(
        search.more_like_this(MockItem.id, document_id=source_id, fields=["description"])
    )
    assert_uses_paradedb_scan(mock_session, stmt, index_name="mock_items_bm25_idx")
    ids = _ids(mock_session, stmt)
...
def test_more_like_this_by_document_ids(mock_session):
    ...
    shoe_items = list(mock_session.scalars(
        select(MockItem.id).where(search.match_any(MockItem.description, "shoes")).limit(1)
    ))
    wireless_items = list(mock_session.scalars(
        select(MockItem.id).where(search.match_any(MockItem.description, "wireless")).limit(1)
    ))
    ...
    stmt_id1 = select(MockItem.id).where(
        search.more_like_this(MockItem.id, document_id=id1, fields=["description"])
    )
    stmt_id2 = select(MockItem.id).where(
        search.more_like_this(MockItem.id, document_id=id2, fields=["description"])
    )
...
def test_more_like_this_with_stopwords(mock_session):
    ...
    ids_baseline = _ids(mock_session, select(MockItem.id).where(
        search.more_like_this(MockItem.id, document_id=source_id, fields=["description"])
    ))
    ids_with_stopword = _ids(mock_session, select(MockItem.id).where(
...
def test_more_like_this_with_min_word_length(mock_session):
    ...
    ids_filtered = _ids(mock_session, select(MockItem.id).where(
...
def test_range_term_with_range_type(mock_session):
    ...
    with engine.begin() as conn:
        conn.execute(
            text(
                "CREATE INDEX rt_items_pq_bm25_idx ON rt_items_pq "
                "USING bm25 (id, weight_range) WITH (key_field='id')"
            )
        )
        conn.execute(
            text(
    ...
    try:
        with mock_session.get_bind().connect() as conn:
            ...
            with S(bind=conn) as s:
                stmt = (
                    select(tbl.c.id)
                    .where(search.range_term(tbl.c.weight_range, "(10, 12]", relation="Intersects", range_type="int4range"))
                    .order_by(tbl.c.id)
                )
...
def test_search_combined_with_standard_filter(mock_session):
    ...
    if ids:
        ratings = list(mock_session.scalars(
            select(MockItem.rating).where(MockItem.id.in_(ids))
        ))
